...
# Persistent storage paths (mounted EFS volume)
DATA_DIR = Path(os.getenv("DATA_DIR", "/data"))
DATA_DIR.mkdir(parents=True, exist_ok=True)

DATABASE_PATH = os.getenv("DATABASE_PATH", str(DATA_DIR / "billings.db"))
...
class BillingData(BaseModel):
    ...
    raw_text: str = ""
    status: str = "pending"  # processing, pending, confirmed, rejected
...
async def cleanup_orphaned_processing():
    """Delete entries stuck in 'processing' state from previous crash/OOM."""
    async with aiosqlite.connect(DATABASE_PATH) as db:
        cursor = await db.execute(
            "SELECT id, filename FROM billings WHERE status = 'processing'"
        )
        orphaned = await cursor.fetchall()
        
        if orphaned:
            print(f"Cleaning up {len(orphaned)} orphaned processing entries...")
            for row in orphaned:
                billing_id, filename = row
                print(f"  Deleting orphaned: {filename} ({billing_id})")
            ...
            await db.execute("DELETE FROM billings WHERE status = 'processing'")
            await db.commit()
            print(f"Cleanup complete.")


async def update_billing_status(billing_id: str, status: str, error_message: str = None):
    """Update billing status and optionally error message."""
    async with aiosqlite.connect(DATABASE_PATH) as db:
        if error_message:
            await db.execute(
                "UPDATE billings SET status = ?, error_message = ? WHERE id = ?",
                (status, error_message, billing_id)
            )
        else:
            await db.execute(
                "UPDATE billings SET status = ? WHERE id = ?",
                (status, billing_id)
            )
        await db.commit()


async def update_billing_full(billing_id: str, data: dict):
    """Update all billing fields after processing."""
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute("""
            UPDATE billings SET 
                vendor = ?, invoice_number = ?, date = ?, due_date = ?,
                subtotal = ?, tax = ?, total = ?, currency = ?,
                line_items = ?, raw_text = ?, status = ?, has_images = ?
            WHERE id = ?
        """, (
            data.get('vendor'), data.get('invoice_number'), data.get('date'), 
            data.get('due_date'), data.get('subtotal'), data.get('tax'), 
            data.get('total'), data.get('currency', 'EUR'),
            json.dumps([item.dict() if hasattr(item, 'dict') else item for item in data.get('line_items', [])]),
            data.get('raw_text', '')[:10000], 'pending', data.get('has_images', False),
            billing_id
        ))
        await db.commit()
    ...
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute("""
            INSERT INTO billings (id, filename, file_type, vendor, invoice_number, date, due_date,
                                  ...
                                  status, has_images, created_at, confirmed_at, error_message)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            ...
            data.created_at, data.confirmed_at, data.error_message
        ))
        await db.commit()
...
def process_images_in_pdf(pdf_path: str) -> dict:
    """
    Extract image metadata from PDF for billing document analysis.
    
    Purpose:
        Many billing PDFs contain embedded images (logos, stamps, scanned receipts).
        This function inventories all images and extracts metadata to help the AI
        understand what visual content exists in the document.
    
    Returns:
        dict with:
        - count: number of images found
        - extracted_text: list of image descriptions (dimensions, page location)
        - total_pixels: sum of all image pixels (for analytics)
    
    Note:
        The returned metadata is appended to raw_text before AI extraction,
            ...
            if page.images:
                for img_info in page.images:
                    ...
                    image_data["extracted_text"].append(
                        f"[Image {image_data['count']}] Page {page_num + 1}: {width}x{height}px"
                    )
...
def extract_from_pdf(pdf_path: str, filename: str) -> BillingData:
    ...
    # Process images for additional text extraction
    if has_images:
...
async def startup():
    await init_db()
    await cleanup_orphaned_processing()
...
def process_file_sync(billing_id: str, file_path: str, filename: str, is_pdf: bool):
    """Process file in background thread (sync wrapper for async DB ops)."""
    import asyncio
    
    async def do_process():
        try:
            if is_pdf:
                billing = extract_from_pdf(file_path, filename)
            else:
                billing = extract_from_image(file_path, filename)
            
            # Update the billing record with extracted data
            await update_billing_full(billing_id, {
                ...
                'raw_text': billing.raw_text,
                'has_images': billing.has_images,
            })
            print(f"✅ Processing complete for {billing_id}")
            
        except Exception as e:
            print(f"❌ Processing failed for {billing_id}: {e}")
            await update_billing_status(billing_id, "error", str(e))
    ...
    # Run async code in new event loop (since we're in a thread)
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        loop.run_until_complete(do_process())
    finally:
        loop.close()
...
async def upload_billing(file: UploadFile = File(...)):
    """Upload a billing PDF or image. Processing happens in background."""
    ...
    await save_billing(placeholder)
    
    # Start background processing in a thread
    thread = threading.Thread(
        target=process_file_sync,
        args=(billing_id, str(file_path), file.filename, is_pdf)
    )
    thread.start()
