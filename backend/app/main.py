...
def process_images_in_pdf(pdf_path: str) -> dict:
    ...
    🐛 BUG: Collects ALL images into memory before processing - causes OOM!
    """
    image_data = {"count": 0, "extracted_text": []}
    collected_images = []  # 🐛 BUG: Stores all raw pixel data in memory
        ...
        for page_num, page in enumerate(pdf.pages):
            if page.images:
                for img_info in page.images:
                    ...
                    # 🐛 BUG: Convert to RGBA and store raw bytes - HUGE memory usage!
                    # A single 1000x1000 image = 4MB of raw pixel data
                    raw_pixels = pil_img.convert("RGBA").tobytes()
                    collected_images.append(raw_pixels)  # 🐛 Keeps growing!

                    image_data["count"] += 1
                    image_data["extracted_text"].append(
                        f"Image {image_data['count']}: {pil_img.size[0]}x{pil_img.size[1]} (page {page_num + 1})"
                    )

    # 🐛 BUG: All images now in memory - will OOM on PDFs with many/large images
    image_data["total_raw_bytes"] = sum(len(img) for img in collected_images)
    return image_data
...
def extract_from_pdf(pdf_path: str, filename: str) -> BillingData:
    ...
    # 🐛 BUG: Process all images in memory - causes OOM on large PDFs
    if has_images:
